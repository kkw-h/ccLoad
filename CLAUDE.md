...
- **Cursor CLI 契约**(`cursorauth/`+`app/cursor_wire.go`+`app/cursor_credentials.go`+`app/admin_cursor_{auth,oauth}.go`):`auth_type=cursor_oauth`。登录是轮询式 PKCE:`POST /admin/cursor/oauth/start` 本地生成 `uuid+verifier`，浏览器打开 `https://cursor.com/loginDeepControl?challenge&uuid&mode=login&redirectTarget=cli`，服务端轮询 `GET https://api2.cursor.sh/auth/poll?uuid&verifier`（404=pending，200 带 `accessToken/refreshToken`=ready）。也可 `POST /admin/cursor/credentials/import` 导入 User API Key（`POST /auth/exchange_user_api_key`）或直接粘贴 `accessToken`。身份/目录/额度走 Connect JSON 一元 RPC：`DashboardService/GetMe`、`AgentService/GetUsableModels`、`DashboardService/GetCurrentPeriodUsage`（主窗口用 `totalSpend/limit`，不要用 `totalPercentUsed`）。渠道 URL 固定 `https://api2.cursor.sh`，协议 `anthropic,openai` + local 转换。推理**不 HTTP 转发**：`tryCursorOAuthChannel` 写临时 `XDG_CONFIG_HOME/cursor/auth.json` 后 spawn `cursor-agent --print --trust --mode ask --output-format stream-json`；CLI 不在 PATH 时 503。ask 模式是只读的，避免 Cursor 在网关本机执行 shell/写文件。客户端 `tools`/`tool_use`/`function_call`/`tool_result` 走 prompt 映射：把工具目录写进 prompt，模型输出 `<cc_tool_call>{"name","arguments"}</cc_tool_call>`，再译回 Anthropic `tool_use` 或 OpenAI `tool_calls`；下一轮把 `tool_result`/`role=tool` 写回 prompt。工具名按家族对齐 Grok Build(`run_terminal_command`/`read_file`/`search_replace`)、OpenCode(`bash`/`read`/`edit`/`apply_patch`)、Codex CLI(`shell`/`apply_patch`) 以及 Claude Code/Cursor 别名，参数键也按客户端 schema 改写（`cmd`→`command`，`path`→`file_path`/`target_file`/`filePath`）。不实现 RunSSE protobuf 工具循环，也不在网关执行客户端工具。额度快照写入凭证 `oauth_usage`，不记 `quota_cost_usage`
...
- **模型名思考后缀**(`thinking_suffix.go`+`model/thinking_suffix.go`):`gpt-5.6-luna(max)` 只是「客户端在 body 里写思考参数」的语法糖。`applyThinkingSuffix` 在**客户端协议的原始 body** 上改写一次,位置在协议转换**之前**,共三个入口:HTTP 代理 handler、Responses WS turn(完整 transcript 与增量体都要写)、管理测试 `buildChannelTestRequestPlan`——三处必须同步登记,漏一处该入口的后缀就是静默空操作。跨协议管理测试还要在 `upstreamTester.Build` 之后再写一次上游协议 body,否则 `patchUpstreamTestFields` 会用 Codex 模板默认 `reasoning.effort=medium` 盖掉后缀。**别挪进 `prepareTranslatedUpstreamBody`**:那里的 body 形态取决于上游,Antigravity 的 `{"request":{...}}` 信封会把顶层字段整段丢弃。跨协议映射与模型能力裁剪归 registry 转换器;同协议直通只按客户端协议枚举收敛,不在选路前查 catalog(Anthropic 等级走 `adaptive`+`MapToClaudeEffort`,数字预算走 `enabled`+`budget_tokens`;Gemini 收敛到 low/medium/high,`none` 用 `thinkingBudget=0`;OpenAI/Codex 的 `auto` 删字段,`max` 写成 `xhigh`)。后缀不是模型身份:`model.RoutingModelName` 是唯一剥离入口,`GetModels()`/`FuzzyMatchModel`/模型索引按基名归一(条目字面写 `x(max)` 也能按 `x` 路由,显式基名条目优先不被别名覆盖),选路/鉴权/冷却/上游模型名与路径一律基名。渠道 `custom_request_rules` 在上游边界应用,晚于后缀,所以渠道规则覆盖后缀——与其它 body 字段一致

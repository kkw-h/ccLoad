...
func TestConvertGeminiResponseToOpenAIResponses_DoneCompletesUnsignedReasoning(t *testing.T) {
	var param any
	var out [][]byte
	...
	if addedSignature != "" || strings.Join(deltas, "") != "unsigned thought" || doneCount != 1 || completedCount != 1 {
		t.Fatalf("DONE completion malformed: added=%q deltas=%q done=%d completed=%d", addedSignature, deltas, doneCount, completedCount)
	}

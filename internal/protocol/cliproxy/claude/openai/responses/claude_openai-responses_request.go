...
func ConvertOpenAIResponsesRequestToClaude(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	setInstructions := func(text string) {
		if text == "" {
			return
		}
		if hasConversationInput {
			system := []byte(`[{"type":"text","text":""}]`)
			system, _ = sjson.SetBytes(system, "0.text", text)
			out, _ = sjson.SetRawBytes(out, "system", system)
			return
		}
	...
	// instructions -> Anthropic system field when a conversation exists;
	// instructions-only requests become the sole user prompt.
	instructionsText := ""

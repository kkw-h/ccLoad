...
func (s *Server) forwardCursorAgent(
	...
	if err != nil {
		...
		if !reqCtx.skipProxyLog {
			...
			if status != StatusClientClosedRequest {
				s.updateTokenStatsForProxy(reqCtx, cfg, model.TokenStatFailure(), duration, failed, modelID)
			}
		}
		return result, nil
	}
	...
	if !reqCtx.skipProxyLog && !replayed {
		...
		s.updateTokenStatsForProxy(reqCtx, cfg, model.TokenStatSuccess(), duration, forwarded, modelID)
	}
	return &proxyResult{

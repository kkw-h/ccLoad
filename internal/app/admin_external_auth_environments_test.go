...
func TestAdminExternalAuthEnvironmentRoutesAreRegistered(t *testing.T) {
	server := newInMemoryServer(t)
	engine := gin.New()
	server.SetupRoutes(engine)

	want := map[string]bool{
		http.MethodGet + " /admin/external-auth-environments":        false,
		http.MethodPost + " /admin/external-auth-environments":       false,
		http.MethodPut + " /admin/external-auth-environments/:id":    false,
		http.MethodDelete + " /admin/external-auth-environments/:id": false,
	}
